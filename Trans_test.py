...
with open(dataset, "r") as file:
    data=json.load(file)
org_context = [i['context'] for i in data]
org_question = [i['question'] for i in data]
...
# Translate the context and question to eng from source language.
tar_lan = dataset[-13:-5]
context = test.translate_in_to_eng(org_context,tar_lan,"eng_Latn")
question = test.translate_in_to_eng(org_question,tar_lan,"eng_Latn")
...
# Loading Model and Performing inference with VLLM.           
sampling_params = SamplingParams(temperature=0.0, top_p=0.95)
llm = LLM(model=model_name,device='auto',download_dir=local_data_dir,tensor_parallel_size=1)
answer=[]
prediction=[]
...
# Deleting Previous model in the pipeline.
del llm
gc.collect()
torch.cuda.empty_cache()

# Backtranslating the Generated response from LLM to source language.
output=test.translate_eng_to_in(prediction,"eng_Latn",tar_lan)
...
with open(f'{directory_path}/Results_trans/{filename}_.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['ground_truth','answer'])
    writer.writerows(combined_lists)

#storing the Quantative results.
